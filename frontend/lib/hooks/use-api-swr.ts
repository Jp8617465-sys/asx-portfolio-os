...
export function useLiveSignals(
  ...
  return useSWR(
    `/api/signals/live?model=${model}&limit=${limit}`,
    fetcher,
    {
...
export function useSignal(ticker: string | null, config?: SWRConfiguration) {
  return useSWR(
    ticker ? `/api/signals/${ticker}` : null,
    fetcher,
    { dedupingInterval: 10_000, ...config }
  );
}

export function useSignalReasoning(ticker: string | null, config?: SWRConfiguration) {
  return useSWR(
    ticker ? `/api/signals/${ticker}/reasoning` : null,
    fetcher,
    { dedupingInterval: 30_000, ...config }
  );
}
...
export function useEnsembleSignals(
  ...
  return useSWR(
    `/api/signals/ensemble/latest${qs ? `?${qs}` : ''}`,
    fetcher,
    { dedupingInterval: 10_000, ...config }
  );
}
...
export function usePriceHistory(
  ...
  return useSWR(
    ticker ? `/api/prices/${ticker}/history?period=${period}` : null,
    fetcher,
    { dedupingInterval: 30_000, revalidateOnFocus: false, ...config }
  );
}
...
export function useModelStatusSummary(model = 'model_a_ml', config?: SWRConfiguration) {
  return useSWR(
    `/api/model/status/summary?model=${model}`,
    fetcher,
    { dedupingInterval: 30_000, ...config }
  );
}
...
export function useDriftSummary(model = 'model_a_ml', config?: SWRConfiguration) {
  return useSWR(
    `/api/drift/summary?model=${model}`,
    fetcher,
    { dedupingInterval: 60_000, ...config }
  );
}
...
export function useAccuracy(ticker: string | null, config?: SWRConfiguration) {
  return useSWR(
    ticker ? `/api/accuracy/${ticker}` : null,
    fetcher,
